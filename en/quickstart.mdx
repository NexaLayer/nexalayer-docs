...
If the same email requests again, NexaLayer resends the existing API key. You can top up with USDT (minimum ¥10) and use NexaLayer directly. For a trial, contact [Telegram support](https://t.me/ZTPROXY) (@ztproxy), who will configure resources for the matching email account.
...
`provisioning_mode=existing` attaches a resource already configured by support without placing an order or charging balance. Use `purchase` after recharging when you want to buy a new resource. Prices and balance come from the live CNY values returned by `/products` and `/billing/balance`; NexaLayer does not make a second debit.
