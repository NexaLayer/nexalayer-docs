...
如果同一个邮箱再次请求，系统会重发已有 API Key。你可以通过 USDT TRC20 自助充值（最低 $10）后直接使用；如需试用，请联系 [Telegram 售后](https://t.me/ZTPROXY)（@ztproxy），由售后为对应邮箱账号配置资源。
...
`provisioning_mode=existing` 会直接使用售后已配置的账号资源，不下单或扣费。自行充值并购买新资源时改为 `purchase`；购买价格和余额使用 IPProxy 当前有效汇率换算为 USD 展示，NexaLayer 不会再次预扣。
