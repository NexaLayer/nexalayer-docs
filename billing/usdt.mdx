---
title: USDT 充值
description: ZTProxy USDT TRC20 充值订单接口。基础地址：https://backend.ztproxy.net/api
---
...
```bash
curl -X GET "https://backend.ztproxy.net/api/usdt/wallet-info" \
...
```bash
curl -X POST "https://backend.ztproxy.net/api/usdt/create-order" \
  -H "Authorization: Bearer <token>" \
...
```bash
curl -X GET "https://backend.ztproxy.net/api/usdt/check-status?order_id=USDT202605260001" \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X GET "https://backend.ztproxy.net/api/usdt/orders?page=1&page_size=10&status=pending" \
  -H "Authorization: Bearer <token>"
```
...
```bash
curl -X POST "https://backend.ztproxy.net/api/usdt/cancel-order" \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{
    "order_id": "USDT202605260001"
