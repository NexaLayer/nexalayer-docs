...
|------|--------|-------------|
| Dynamic | [Dynamic](/docs/sessions/dynamic) | Rotating IP; pay per traffic (GB). Create with `type: "dynamic"` and `config.product_no`, `config.traffic_gb`, etc. |
| Static | [Static](/docs/sessions/static) | Fixed IP; pay per duration. Create with `type: "static"` and `config.product_no`, `config.quantity`, `config.duration_days`. |
...
Next: [Dynamic](/docs/sessions/dynamic), [Static](/docs/sessions/static), [Rotate](/docs/sessions/rotate), [Renew](/docs/sessions/renew), [Terminate](/docs/sessions/terminate).
