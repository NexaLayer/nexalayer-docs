---
title: Quickstart
description: "Register, create a session, and send traffic in under 5 minutes. Base URL: https://agent-layer.up.railway.app/v1"
---
...
Call the register API once to get credentials. Store `api_key` for all subsequent requests.

<CodeGroup>

```bash Request
curl -X POST "https://agent-layer.up.railway.app/v1/account/register" \
...
BASE_URL = "https://agent-layer.up.railway.app/v1"
...
const BASE_URL = 'https://agent-layer.up.railway.app/v1';
...
Next: [Authentication](/docs/docs/authentication) (API Key vs JWT), [Sessions](/docs/docs/sessions/overview) (dynamic vs static, rotate, renew, terminate).
