...
响应项通常包含 `productId`、`name`、`proxyType`、`stock`、`countryCode`、`countryName`、`countryNameEn`、`countryNameZh`、`cityCode`、`cityName`、`cityNameEn`、`cityNameZh`、`nameLanguage`、`duration`、`unit` 和 `price` 等字段。
...
响应数据位于 `data.items`，每个产品项包含 `countryName`、`countryNameEn`、`countryNameZh`、`stateName`、`cityName`、`cityNameEn`、`cityNameZh`、`nameLanguage` 和 `ipRanges`。`ipRanges[].ipRange` 即可用 CIDR/IP 段。
