...
响应项通常包含 `productId`、`name`、`proxyType`、`stock`、`area` / `areaCode`、`country` / `countryCode`、`state` / `stateCode`、`city` / `cityCode`、`duration`、`unit` 和 `price` 等字段。

### 地区字段说明

静态 `103` 产品的地区代码来自 `POST /open/app/product/query/v2` 响应项。下单时请使用同一个产品返回的 `areaCode`、`countryCode` 和 `cityCode`，不要跨产品复用地区代码。
